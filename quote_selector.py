...
def extract_quotes_from_pdf(pdf_path: Path, gap_multiplier: float = 1.8) -> List[str]:
    ...
    # Drop a title line if it appears as a “paragraph” in the PDF
    paragraphs = [p for p in paragraphs if p.strip() and not p.strip().lower().startswith("passages -")]
...
def load_all_quotes(
    ...
    for pdf_path in pdfs:
        ...
        if use_cache and cached and cached.get("mtime") == mtime and cached.get("gap_multiplier") == gap_multiplier:
...
def pick_random_quote(
    ...
    quotes = load_all_quotes(folder, pattern=pattern, recursive=recursive, gap_multiplier=gap_multiplier, use_cache=True)
    if not quotes:
